...
def main():
    ...
    # evaluation metrics for korquad
    # load squad_kor_v1(v2)_local evaluation metric
    current_dir = os.path.sep.join(os.path.join(__file__).split(os.path.sep)[:-1])
    metric_path = (
        "squad_kor_v2_local"
        if data_args.version_2_with_negative
        else "squad_kor_v1_local"
    )
    metric = load_metric(os.path.join(current_dir, metric_path))
